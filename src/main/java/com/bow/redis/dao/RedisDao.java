...
import com.bow.redis.annation.RedisFieldNotCache;
import com.bow.redis.annation.RedisQuery;
import com.bow.redis.util.BeanField;
...
public class RedisDao {
    ...
    /**
     * 初始化将表中数据放到redis,存放格式为tableName:id:column.
     * 
    ...
    /**
     * 初始化存放表中所有字段数据，存放格式为tableName:column:columnValue
     * 
     * @param tableName
     * @param column
     * @param id
    ...
    /**
     * 初始化将表中数据放到redis,将bean转换成json格式，存放格式为tableName:id。
     * 
     * @param tableName 表映射的类名
     * @param id 主键id值
    ...
    /**
     * 根据jsonKey，获取相应的json字符串，转换成实体类List
     */
    ...
    public void delSingleDataFromRedis(Object t, BeanField bf) throws Exception {
        ...
        for (Field field : fieldList) {
            ...
            if (null != fieldValue && !field.isAnnotationPresent(RedisFieldNotCache.class)) {
                ...
                if (field.isAnnotationPresent(RedisQuery.class)) {
                    // 2、删除类型2
                    delSet(className + SPLIT_MARK + fieldName + SPLIT_MARK + fieldValue.toString(),
    ...
    public void updateSingleFromToredis(Object oldObject, Object t, BeanField bf)
        ...
        for (Field field : fieldList) {
            ...
            if (null != fieldValue && !field.isAnnotationPresent(RedisFieldNotCache.class)) {
                ...
                if (field.isAnnotationPresent(RedisQuery.class)) {
                    // 删除就的类型2
                    delSet(className + SPLIT_MARK + fieldName + SPLIT_MARK + field.get(oldObject),
                            className + SPLIT_MARK + primaryValue);
                    // 2、更新类型二 k/v
                    saddColumn(className, fieldName, fieldValue.toString(), primaryValue);
                }
